...
간단한 Streamlit UI에서 PDF를 업로드하면 먼저 목차를 파악하고, 사용자가 선택한 항목의 페이지 범위만 분석한 뒤 `opendataloader-pdf` 파서가 추출한 테이블을 HTML 또는 Markdown 형식으로 확인하고 내보내는 예제 앱입니다.
...
2. 앱이 PDF 내장 목차를 먼저 읽고, 목차가 없으면 페이지별 제목 후보를 추정합니다.
3. 분석할 목차 항목을 선택합니다. 필요하면 페이지 범위를 직접 입력해 선택 범위를 덮어쓸 수 있습니다.
4. `분석 시작`을 눌러 선택한 항목의 페이지 범위만 파싱합니다.
5. 분석이 완료되면 가벼운 테이블 인덱스에서 크기와 식별 단서를 확인합니다.
6. 테이블 내용 조건을 입력해 원하는 단어 또는 문구가 포함된 테이블만 좁힙니다.
7. 필요한 경우 테이블 하나만 상세 미리보기로 열어 내용을 확인합니다.
8. 내보낼 테이블만 선택하고 HTML 또는 Markdown 형식으로 파일을 저장합니다.
9. 파싱 중 오류가 발생하면 화면에 친화적인 에러 메시지가 표시됩니다.
