...
간단한 Streamlit UI에서 PDF를 업로드하고 원하는 페이지만 분석한 뒤, `opendataloader-pdf` 파서가 추출한 테이블을 HTML 또는 Markdown 형식으로 확인하고 내보내는 예제 앱입니다.
...
2. 확인할 페이지 범위를 입력합니다. 예: `1,3,5-7`
3. `분석 시작`을 눌러 선택한 페이지만 파싱합니다.
4. 분석이 완료되면 가벼운 테이블 인덱스에서 크기와 식별 단서를 확인합니다.
5. 필요한 경우 테이블 하나만 상세 미리보기로 열어 내용을 확인합니다.
6. 내보낼 테이블만 선택하고 HTML 또는 Markdown 형식으로 파일을 저장합니다.
7. 파싱 중 오류가 발생하면 화면에 친화적인 에러 메시지가 표시됩니다.
